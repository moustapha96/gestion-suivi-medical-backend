...
import com.ut.sn.DTO.ConsultationDTO;
...
public class DossierMedicalController {
	...
	@Autowired ConsultationRepository consRepo;
	@Autowired
	...
	@PostMapping("/dms/consultation/{id}")
	public Dossier_medical ajouterConsultationAuDm(@PathVariable(value = "id") Integer Id , 
					@RequestBody ConsultationDTO constdto) throws ResourceNotFoundException {
		
		Dossier_medical dossier_medicalV = repository.findById(Id)
				.orElseThrow(() -> new ResourceNotFoundException("dossier_medical non trouvé  :: " + Id));
		
		Consultation consultation = new Consultation(constdto.getDiagnostic(), constdto.getTraitement());
		consultation.setDossierMedical(dossier_medicalV);
		
		Collection<Consultation> consultations = dossier_medicalV.getConsultations();
		
		consultations.add(consultation);
		
		dossier_medicalV.setConsultations(consultations);

	    repository.save(dossier_medicalV);
		
		return dossier_medicalV;
	}
...
//		return ResponseEntity.ok().body(dossier_medical);
	...
	@GetMapping("/dms/patientDM/{id}")
	public ResponseEntity<Dossier_medical> getDossierMedicalPatient(@PathVariable("id") Integer id)
			throws ResourceNotFoundException {
		
		 Patient patient = new Patient();
			Optional<Patient> pa = patientRepo.findByIdPatient(id);
			if(pa.isPresent()){
				 patient = pa.get();
			}
		
		Dossier_medical dm = repository.findDmByPatient(patient);
